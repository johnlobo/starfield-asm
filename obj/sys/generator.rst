...
Hexadecimal [16-Bits]
                              ...
                              5 
   018A 00                    6 counter: .db 0
                              7 
                              8 ;;
                              9 ;;  When first called, takes address of first entity 
                             10 ;;
   018B                      11 p_getFirstEntityPointer:
   0001                      12     ld__a_ixl
   018B DD 7D                 1    .dw #0x7DDD  ;; Opcode for ld a, ixl
   018D 32 A7 01      [13]   13     ld (__firste_l), a
   0006                      14     ld__a_ixh
   0190 DD 7C                 1    .dw #0x7CDD  ;; Opcode for ld a, ixh
   0192 32 AD 01      [13]   15     ld (__firste_h),a
   0195 21 8A 01      [10]   16     ld hl, #counter
   0198 36 01         [10]   17     ld (hl), #1
   019A 21 A1 01      [10]   18     ld hl, #p_countEntities
   019D 22 01 02      [16]   19     ld (sys_generator_update+1), hl
   01A0 C9            [10]   20     ret
                             21 
                             22 ;;
                             23 ;; Counts Entities
                             24 ;;
   01A1                      25 p_countEntities:
   01A1 21 8A 01      [10]   26     ld hl, #counter
                             27 
                             28     ;; Check if this is the first entity
   001A                      29     ld__a_ixl
   01A4 DD 7D                 1    .dw #0x7DDD  ;; Opcode for ld a, ixl
                     001D    30     __firste_l = .+1
   01A6 D6 00         [ 7]   31     sub #00
   01A8 20 06         [12]   32     jr nz, _pce_not_first
   0020                      33     ld__a_ixh
   01AA DD 7C                 1    .dw #0x7CDD  ;; Opcode for ld a, ixh
                     0023    34     __firste_h = .+1
   01AC D6 00         [ 7]   35     sub #00
   01AE 28 02         [12]   36     jr z, _pce_first
   01B0                      37 _pce_not_first:
   01B0 34            [11]   38     inc(hl)
   01B1 C9            [10]   39     ret
   01B2                      40 _pce_first:
                             41 ;; Check if there is less than constant_stars value
   01B2 3E 04         [ 7]   42     ld a, #constant_stars
   01B4 96            [ 7]   43     sub (hl)
   01B5 36 01         [10]   44     ld (hl), #1
   01B7 C8            [11]   45     ret z ;; Maximun number of stars complete
   01B8 D8            [11]   46     ret c
   01B9                      47 _pce_create_stars:
                             48 ;; Save IX will be modifying it
   01B9 DD E5         [15]   49     push IX
                             50     ;; set new stars to be created (up to three)
   01BB FE 03         [ 7]   51     cp #3
   01BD 38 02         [12]   52     jr c, _pce_star_n_ok
   01BF 3E 03         [ 7]   53     ld a, #3
   01C1                      54 _pce_star_n_ok:
   01C1 32 F4 01      [13]   55     ld (__star_cnt), a
...
Hexadecimal [16-Bits]
                             ...
                             56 
   01C4                      57 _pce_create_next_star:
                             58 ;; Create new stars
   01C4 CD C4 00      [17]   59     call man_entity_create
   003D                      60     ld__ixl_e ;; IX = DE (Entity created)
   01C7 DD 6B                 1    .dw #0x6BDD  ;; Opcode for ld ixl, e
   003F                      61     ld__ixh_d
   01C9 DD 62                 1    .dw #0x62DD  ;; Opcode for ld ixh, d
                             62     ;; Set default values
   01CB DD 36 00 01   [19]   63     ld e_type(ix), #e_type_star
   01CF DD 36 01 4F   [19]   64     ld e_x(ix), #79
   01D3 DD 36 05 01   [19]   65     ld e_color(ix), #e_color_default
   01D7 DD 36 06 00   [19]   66     ld e_prevptr(ix), #0x00
   01DB DD 36 07 00   [19]   67     ld e_prevptr+1(ix), #0x00
                             68     ;; Set random y (0..199)
                             69 
   01DF                      70 _pce_invalid_y:
   01DF CD 94 02      [17]   71     call cpct_getRandom_xsp40_u8_asm
   01E2 FE C7         [ 7]   72     cp #199
   01E4 30 F9         [12]   73     jr nc, _pce_invalid_y
   01E6 DD 77 02      [19]   74     ld e_y(ix), a
                             75 
                             76     ;; Set random vx (-1..-4)
   01E9 CD 94 02      [17]   77     call cpct_getRandom_xsp40_u8_asm
   01EC E6 03         [ 7]   78     and #0x03
   01EE ED 44         [ 8]   79     neg
   01F0 DD 77 03      [19]   80     ld e_vx(ix), a
                             81 
                             82     ;; Counter stars to be created
                     006A    83     __star_cnt = . + 1
   01F3 3E 03         [ 7]   84     ld a, #3
   01F5 3D            [ 4]   85     dec a
   01F6 28 05         [12]   86     jr z, _pce_return
   01F8 32 F4 01      [13]   87     ld (__star_cnt), a
   01FB 18 C7         [12]   88     jr _pce_create_next_star
   01FD                      89 _pce_return:
   01FD DD E1         [14]   90     pop ix
   01FF C9            [10]   91     ret
                             92 
                             93 ;;
                             94 ;; UPDATE
                             95 ;;  Generates new stars whenever required
                             96 ;; Input
                             97 ;;  IX : Particle (entity)
   0200                      98 sys_generator_update::
   0200 C3 8B 01      [10]   99     jp p_getFirstEntityPointer  ;; Jumps to current
