...
Hexadecimal [16-Bits]
                              ...
                              6 ;;
   0069                       7 entities::
   0069 6B 00                 8 entities_1st_free:  .dw entities_storage
   006B                       9 entities_storage:   .ds max_entities * sizeof_e
                             10 
                             11 ;;
                             12 ;; Initialize entity manager
                             13 ;;
   00BB                      14 man_entity_init::
                             15     ;; 1st free entity at the start of entities storage
   00BB 21 6B 00      [10]   16     ld hl, #entities_storage
   00BE 22 69 00      [16]   17     ld (entities_1st_free), hl
                             18     ;; 1st free entity with a 0 as first byte (no-entity)
   00C1 36 00         [10]   19     ld (hl), #0
   00C3 C9            [10]   20     ret
                             21 
                             22 ;; CREATE A NEW ENTITY
                             23 ;;  Returns: DE = New entity pointer
                             24 ;;
                             25 ;;
   00C4                      26 man_entity_create::
   00C4 2A 69 00      [16]   27     ld hl, (entities_1st_free)  ;; HL Points to first free entity
   00C7 54            [ 4]   28     ld d, h
   00C8 5D            [ 4]   29     ld e, l                     ;; DE copy of HL
                             30 
                             31     ;; Fill the entity
   00C9 36 00         [10]   32     ld (hl), #e_type_default    ;; Test alive entity
                             33 
                             34     ;; Update entities_1st_free to next location
   00CB 01 08 00      [10]   35     ld bc, #sizeof_e
   00CE 09            [11]   36     add hl, bc
   00CF 22 69 00      [16]   37     ld (entities_1st_free), hl
                             38 
   00D2 C9            [10]   39     ret
                             40 
                             41 ;;
                             42 ;; SET an entity for destruction
                             43 ;;  Marks an entity for later destruction
                             44 ;; Input:
                             45 ;;  IX: Points to Entity to be destroyed
                             46 ;;
   00D3                      47 man_entity_set4destruction::
   00D3 DD 7E 00      [19]   48     ld a, e_type(ix)
   00D6 F6 80         [ 7]   49     or #e_type_dead      ;; Add dead bit
   00D8 DD 77 00      [19]   50     ld e_type(ix), a    ;; and store
   00DB C9            [10]   51     ret
                             52 
                             53 ;;
                             54 ;; DESTROY an entity
                             55 ;;  Note: Does nt check anything
                             56 ;; Input:
                             57 ;;  IX: Points to entity to be removed
...
Hexadecimal [16-Bits]
                             ...
                             58 ;;
   00DC                      59 man_entity_destroy::
                             60     ;;Update entitues 1st free to prev location
   00DC 2A 69 00      [16]   61     ld hl, (entities_1st_free)
   00DF 01 F8 FF      [10]   62     ld bc, #-sizeof_e
   00E2 09            [11]   63     add hl, bc
   00E3 22 69 00      [16]   64     ld (entities_1st_free), hl
                             65     ;; Check against IX (if they are equal, no copy)
   007D                      66     ld__a_ixl
   00E6 DD 7D                 1    .dw #0x7DDD  ;; Opcode for ld a, ixl
   00E8 95            [ 4]   67     sub l
   00E9 20 05         [12]   68     jr nz, _copy
   0082                      69     ld__a_ixh
   00EB DD 7C                 1    .dw #0x7CDD  ;; Opcode for ld a, ixh
   00ED 94            [ 4]   70     sub h
   00EE 28 09         [12]   71     jr z, _nocopy
   00F0                      72 _copy:
                             73     ;; Copy last entity in the array to remove entity
   0087                      74     ld__e_ixl
   00F0 DD 5D                 1    .dw #0x5DDD  ;; Opcode for ld e, ixl
   0089                      75     ld__d_ixh
   00F2 DD 54                 1    .dw #0x54DD  ;; Opcode for ld d, ixh
   00F4 01 08 00      [10]   76     ld bc, #sizeof_e
   00F7 ED B0         [21]   77     ldir
   00F9                      78 _nocopy:
                             79     ;; Remove las entity from the array, but setting
   00F9 2A 69 00      [16]   80     ld hl, (entities_1st_free)
   00FC 36 00         [10]   81     ld (hl), #0
   00FE C9            [10]   82     ret
                             83 
                             84 ;;
                             85 ;; CALL a function for all entities
                             86 ;; Input:
                             87 ;;  HL: Pointer to the function to be called
                             88 ;;      Important: Function at HL must preserve IX
                             89 ;;
   00FF                      90 man_entity_forall::
   00FF 22 0C 01      [16]   91     ld (_to_call), hl       ;; Set function to be called
   0102 DD 21 6B 00   [14]   92     ld ix, #entities_storage
                             93     ;; check if entity is valid... 1st byte is 0 => not a valid entry
   0106                      94 _next_entity:
   0106 DD 7E 00      [19]   95     ld a, (ix)
   0109 B7            [ 4]   96     or a
   010A C8            [11]   97     ret z
                             98     ;; Entity is valid => call the function
                     00A3    99 _to_call = .+1
   010B CD 0C 01      [17]  100     call _to_call
                            101     ;; Point to the next entity and repeat
   010E 01 08 00      [10]  102     ld bc, #sizeof_e
   0111 DD 09         [15]  103     add ix, bc
   0113 CD 06 01      [17]  104     call _next_entity
                            105 
                            106 ;;
                            107 ;; UPDATE ENTITY MANAGER
                            108 ;;  updates the manager removing entities marked for destruction
...
Hexadecimal [16-Bits]
                            ...
                            109 ;;
   0116                     110 man_entity_update::
   0116 DD 21 6B 00   [14]  111     ld ix, #entities_storage
   011A 18 03         [12]  112     jr _nexte
   011C                     113 _destroy_e:
   011C CD DC 00      [17]  114     call man_entity_destroy
   011F                     115 _nexte:
                            116     ;; Check entity type
   011F DD 7E 00      [19]  117     ld a, e_type(ix)
   0122 B7            [ 4]  118     or a
   0123 C8            [11]  119     ret z
                            120     ;; if it is dead, destroy de entity
   0124 E6 80         [ 7]  121     and #e_type_dead
   0126 20 F4         [12]  122     jr nz, _destroy_e
   0128                     123 _dont_delete_e:
   0128 01 08 00      [10]  124     ld bc, #sizeof_e
   012B DD 09         [15]  125     add ix, bc
   012D 18 F0         [12]  126     jr _nexte
                            127 
                            128 
